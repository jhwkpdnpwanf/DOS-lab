...
이번 실습의 핵심은 네트워크 경계 보안 장비가 공격을 어떻게 인지하고 차단할 수 있는지를 확인하는 데 있습니다. 이를 위해 다음과 같이 환경을 구성합니다.  
...
- Kali Linux (공격자): 외부 네트워크(WAN)에 위치하며, Metasploit을 사용하여 변조된 SYN 패킷을 대량으로 생성합니다.  

- OpenWrt (방화벽/게이트웨이): 외부와 내부 네트워크 사이의 길목을 지키는 '보안관' 역할을 합니다. 모든 패킷은 OpenWrt를 통과해야만 서버에 도달하므로, 여기서 트래픽 모니터링과 탐지 규칙(Rule) 적용을 실습합니다.  

- Metasploitable2 (공격 대상): 내부 네트워크(LAN)에 보호되고 있는 서버입니다. 보안 설정을 변경해가며 방어 유무에 따른 가용성 변화를 테스트합니다.  
...
LAN으로 묶였던 포트를 eth0에서 eth1로 바꿔주고, eth0은 WAN 인터페이스로 사용해줍니다.  

일반적으로 가상 환경에서 첫 번째 네트워크 어댑터(eth0)는 외부와 통신이 가능한 NAT(혹은 Bridged) 방식으로 설정되는 경우가 많습니다. 따라서 이를 외부 진입점(WAN)으로 잡고, Host-Only로 설정된 두번째 어댑터(eth1)를 내부망(LAN)으로 구성하였습니다.  
...
여기서 주의할 점은 이전에 주소에서는 브라우저 캐시가 남아있어, 이전 openwrt의 다른 주소(ex. 관리자 페이지)로 넘어갈 가능성이 큽니다. 이러한 경우에는 시크릿 모드를 사용해 실행해주면 쉽게 올바른 주소로 들어갈 수 있습니다.  
