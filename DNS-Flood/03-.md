...
**netns 세팅**
```
netns resolv_ns     (192.168.125.10)    
...
우선 iproute2 를 설치해주어 netns 4개를 생성해줍니다.  
이름은 각각 `root_ns`, `tld_ns`, `auth_ns`, `resolv_ns`로 설정해주었습니다.  
...
ip link add veth-res  type veth peer name veth-res-br
```
...
ip link set veth-res  netns resolv_ns
```
...
```bash
ip netns exec resolv_ns ip addr add 192.168.125.10/24 dev veth-res
...
ip link set veth-res-br  master br-dns
```


![alt text](./img/NXDOMAIN4.png)   

이렇게 resolv_ns에서 ping을 통해 각 서버와의 통신상태를 확인해보면, 전부 잘 되는 것을 볼 수 있습니다.  
